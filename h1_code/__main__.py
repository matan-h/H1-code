...
def process_files(folder: str, ext: str,lang_name:str,single_backup: bool,config:AiConfig):
    path = Path(folder)
    ...
    for p in track(files,transient=True,description="Converting"):
        ...
        print('----', p.name, '----')
        try:
            h1_code = (ai(source_code, lang_name ,p.relative_to(path),config,StreamLinePrint().add))
...
def main():
    ...
    parser.add_argument('--base_url', type=str, default='http://localhost:11434/v1', help='Base URL for the AI service (default: ollama endpoint [http://localhost:11434/v1] ).')
    parser.add_argument('--api_key', type=str, default='ollama', help='API key for the AI service (default: ollama).')
    ...
    process_files(folder=args.folder, ext=args.ext, lang_name=args.lang_name or args.ext.capitalize(),single_backup=args.single_backup,config=config)
